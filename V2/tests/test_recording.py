...
class TestRecordingFunctionality(unittest.TestCase):
    ...
    def test_server_recording_enabled_with_parameter(self):
        ...
        server = SimpleAudioSocketServer(
            self.test_host, self.test_port, recording_enabled=True
        )
        ...
        self.assertTrue(
            server.recording_enabled, "Server recording should be enabled"
        )
        self.assertTrue(
            hasattr(server, "audio_frames"),
    ...
    def test_server_saves_recording_file_when_enabled(self):
        ...
        for i in range(3):
            ...
            audio_message = AudioSocketProtocol.create_audio_message(
                audio_data
            )
            client_socket.send(audio_message)
            time.sleep(0.05)
        ...
        self.assertTrue(
            recording_completed.wait(timeout=10.0),
            "Recording completion callback should be called",
        )

        # Verify recording file was created
        self.assertIsNotNone(
            recorded_filename, "Recording filename should be provided"
        )
        self.assertTrue(
            os.path.exists(recorded_filename),
        ...
        self.assertTrue(
            recorded_filename.endswith(".wav"), "Filename should end with .wav"
        )
        ...
        with wave.open(recorded_filename, "rb") as wav_file:
            ...
            self.assertEqual(
                wav_file.getsampwidth(), 2, "WAV should be 16-bit"
            )
            self.assertEqual(
                wav_file.getframerate(), 8000, "WAV should be 8kHz"
            )
            self.assertGreater(
                wav_file.getnframes(), 0, "WAV should have audio frames"
            )
    ...
    def test_server_no_recording_when_disabled(self):
        ...
        recordings_before = len(
            [f for f in os.listdir(self.recordings_dir) if f.endswith(".wav")]
        )

        # Wait a bit more to ensure no delayed file creation
        time.sleep(0.5)

        recordings_after = len(
            [f for f in os.listdir(self.recordings_dir) if f.endswith(".wav")]
        )
        self.assertEqual(
            recordings_before,
    ...
    def test_server_recording_callback_functionality(self):
        """Test that server calls the recording completion callback when provided."""
        from simple_server import SimpleAudioSocketServer
        ...
        self.assertTrue(
            callback_called.wait(timeout=10.0),
            "Recording completion callback should be called",
        )

        # Verify callback received correct filename
        self.assertIsNotNone(
            callback_filename, "Callback should receive filename"
        )
        self.assertTrue(
            os.path.exists(callback_filename),
            f"Callback filename should exist: {callback_filename}",
        )
